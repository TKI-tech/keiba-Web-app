...
`render.yaml` は有料の `plan: starter` + 永続ディスク（`/app/data`）を
デフォルトにしてある。実際に会員登録・課金するユーザーがいる前提のため、
(1) 再デプロイ・スリープ復帰のたびに会員データが消える、(2) 15分アクセスが
ないとスリープし次回アクセス時の起動に数十秒かかる、という無料プランの制約は
許容できないという判断による。動作検証だけでよい場合は `plan` を `free` に戻し、
`disk:` セクションをコメントアウトすれば無料でデプロイできる。
...
1. [Render](https://render.com)にログインし、「New +」→「Blueprint」から
   ...
   内容が自動で読み込まれる。
2. デフォルトの `plan: starter` + 永続ディスクには料金がかかる。無料で試したい
   場合は `render.yaml` の `plan` を `free` に変更し、`disk:` セクションを
   コメントアウトしてからpushする（上記の制約は許容する）。
